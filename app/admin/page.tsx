...
import { Plus, Pencil, Trash2, Store, AlertCircle } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <AdminHeader
        title="Dashboard"
        subtitle="Kelola data UMKM Desa Kanten"
        onRefresh={fetchData}
      />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            { label: "Total UMKM",  value: umkmList.length,                                        icon: "🏪" },
            { label: "Featured",    value: umkmList.filter(u => u.featured).length,                icon: "⭐" },
            { label: "RT Terisi",   value: new Set(umkmList.map(u => u.rt)).size,                  icon: "📍" },
            { label: "Kategori",    value: new Set(umkmList.map(u => u.kategori)).size,            icon: "🏷️" },
          ].map(s => (
            <div key={s.label} className="bg-white border border-gray-100 rounded-2xl p-4 text-center shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="text-2xl font-bold text-[#011f6d]">{s.value}</p>
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-gray-900 text-base">Daftar UMKM</h2>
          <Link
            href="/admin/umkm/baru"
            className="flex items-center gap-1.5 bg-[#011f6d] hover:bg-[#1a3d96] text-white text-xs font-semibold px-4 py-2 rounded-xl transition shadow-sm"
          >
        ...
        <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
          {loading ? (
            <div className="py-16 text-center">
              <div className="w-8 h-8 border-2 border-[#011f6d] border-t-transparent rounded-full animate-spin mx-auto mb-3" />
                ...
                {umkmList.map((u) => (
                  ...
                  >
                    <td className="px-5 py-3">
                      <p className="font-semibold text-gray-900 text-sm truncate max-w-[200px]">{u.nama}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{u.whatsapp || "—"}</p>
                    </td>
                    <td className="px-3 py-3 hidden sm:table-cell">
                      <span className="text-xs bg-[#011f6d]/8 text-[#011f6d] px-2 py-1 rounded-full font-medium">
                        {u.rt}
                      </span>
                    </td>
                    <td className="px-3 py-3 hidden sm:table-cell text-xs text-gray-500">{u.kategori}</td>
                    <td className="px-5 py-3">
